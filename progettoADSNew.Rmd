...
```
Prof. Montanari autore più produttivo con 200 articoli
...
Il nodo più centrale della rete risulta essere il Professor Montanari, con i valori più alti di tutti nelle metriche ^[Tutte le nozioni viste a lezione sono utilizzate ma non ri-definite.].
...
Densità: indica una rete sparsa, poco collegata.

Transitività: rete poco clusterizzata.

Centralizzazione del grado: misura che valuta quanto una rete è centralizzata in base alla distribuzione del grado dei nodi. Ci sono alcuni nodi con un numero di collegamenti significativamente maggiore rispetto alla maggior parte degli altri nodi, ma la distribuzione dei gradi non è uguale.
<!-- per essere uguale il valore deve essere 0 -->

Lunghezza media dei percorsi: molto breve, rete compatta e ben collegata.
...
```{r rete cooc stat}
coOcStat <- networkStat(coOc, stat = "all")
summary(coOcStat)
```
...
Centralizzazione del grado: molto basso, distribuzione del grado è quindi abbastanza uniforme, con poche parole chiave dominanti.
...
L'analisi bibliometrica ha evidenziato diversi aspetti importanti, anche attraverso l'analisi delle varie reti.
...
La rete di collaborazione tra autori ha rivelato una struttura coesa con alcuni autori chiave che fungono da nodi centrali. La centralizzazione è moderata, e la rete presenta una buona transitività e un diametro contenuto, suggerendo che gli autori collaborano spesso e che ci sono percorsi relativamente brevi tra di loro.
